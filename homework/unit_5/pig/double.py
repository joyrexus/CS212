...
def pig_actions_d(state):
    ...
    actions = (['accept', 'decline'] if double == "double" else
               ['roll', 'hold'] if pending else ['roll'])
    if double == 1: actions.append('double')
    return actions

def strategy_d(state):
    (_, me, you, pending, double) = state
    chance = Pwin3(me, you, pending)
    if chance > .9 and double == 1:
        return 'double'
    elif pending < goal * .04 or chance < .9:
        return 'roll'
    else:
        return 'hold'
...
def strategy_params(x, y):
    '''
    x parameter indicates percentage chance of winning

    '''
    def f(state):
        (_, me, you, pending, double) = state
        chance = Pwin3(me, you, pending)
        if chance >  x and double == 1:
            return 'double'
        elif pending < goal * y or chance < x:
            return 'roll'
        else:
            return 'hold'
    return f
...
# must win 60% of the points      
# assert strategy_compare(strategy_d, hold_20_d) > 60 

s = strategy_params(.9,.06) 

def strategy_d(state):
    (p, me, you, pending, double) = state
    return ('accept' if double == 'double' else
            "double" if (me + pending >= goal and double == 1) else
            'hold' if (pending >= 20 or double == 2) else
            'roll')
...
for x in range(5, 10):
    for y in range(1, 10):
        X, Y = (x * .1), (y * .01)
        result = strategy_compare(strategy(X,Y), hold_20_d)
        print x, y, result
